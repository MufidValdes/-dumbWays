...
var import_dialect = require("@sequelize/core/_non-semver-use-at-your-own-risk_/utils/dialect.js");
...
var import_data_types_overrides = require("./_internal/data-types-overrides.js");
...
class PostgresQueryGenerator extends import_query_generator_typescript_internal.PostgresQueryGeneratorTypeScript {
  #internals;
  constructor(dialect, internals = new import_query_generator_internal.PostgresQueryGeneratorInternal(dialect)) {
  ...
  createTableQuery(tableName, attributes, options) {
    ...
    if (options.uniqueKeys) {
      (0, import_each.default)(options.uniqueKeys, (index, indexName) => {
    ...
    const pks = (0, import_reduce.default)(
      attributes,
      (acc, attribute, key) => {
  ...
  addColumnQuery(table, key, attribute, options) {
    ...
    if (dataType instanceof import_core.DataTypes.ENUM) {
      query = this.pgEnum(table, key, dataType) + query;
    } else if (dataType instanceof import_core.DataTypes.ARRAY && dataType.options.type instanceof import_core.DataTypes.ENUM) {
  ...
  attributeToSQL(attribute, options) {
    ...
    let type;
    if (attribute.type instanceof import_core.DataTypes.ENUM || attribute.type instanceof import_core.DataTypes.ARRAY && attribute.type.type instanceof import_core.DataTypes.ENUM) {
      const enumType = attribute.type.type || attribute.type;
      const values = enumType.options.values;
      if (Array.isArray(values) && values.length > 0) {
        type = `ENUM(${values.map((value) => this.escape(value)).join(", ")})`;
        if (attribute.type instanceof import_core.DataTypes.ARRAY) {
          type += "[]";
        }
      } else {
    ...
    if ((0, import_query_builder_utils.defaultValueSchemable)(attribute.defaultValue, this.dialect)) {
  ...
  expandOptions(options) {
    return options === void 0 || (0, import_isEmpty.default)(options) ? "" : options.join(" ");
  }
  ...
  expandTriggerEventSpec(fireOnSpec) {
    if ((0, import_isEmpty.default)(fireOnSpec)) {
      throw new Error("no table change events specified to trigger on");
    }
    return (0, import_map.default)(fireOnSpec, (fireValue, fireKey) => {
      const EVENT_MAP = {
        insert: "INSERT",
        update: "UPDATE",
        delete: "DELETE",
  ...
  pgEnum(tableName, attr, dataType, options) {
    ...
    let values;
    if (dataType instanceof import_data_types_overrides.ENUM && dataType.options.values) {
  ...
  fromArray(text) {
    ...
    matches = matches.map(
      (m) => m.replace(/",$/, "").replace(/,$/, "").replaceAll(/(^"|"$)/g, "")
    );
  ...
  quoteIdentifier(identifier, force) {
    ...
    if (optForceQuote === true || // TODO [>7]: drop this.options.quoteIdentifiers. Always quote identifiers based on these rules
    optQuoteIdentifiers !== false || identifier.includes(".") || identifier.includes("->") || POSTGRES_RESERVED_WORDS.includes(identifier.toLowerCase())) {
