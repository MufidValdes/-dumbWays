...
class PostgresQueryGeneratorTypeScript extends import_core.AbstractQueryGenerator {
  #internals;
  ...
  listTablesQuery(options) {
    return (0, import_join_sql_fragments.joinSQLFragments)([
      ...
      options?.schema ? `AND table_schema = ${this.escape(options.schema)}` : `AND table_schema !~ E'^pg_' AND table_schema NOT IN (${this.#internals.getTechnicalSchemaNames().map((schema) => this.escape(schema)).join(", ")})`,
  ...
  getUuidV4FunctionCall() {
    ...
    if (import_semver.default.lt(dialectVersion, "13.0.0")) {
